...
class TSPState:
	def __init__(self, lower_bound, new_matrix, solve=True, new_route=None):
		self.lb = lower_bound
		self.matrix = new_matrix
		self.is_solvable = solve
	...
	def reduce_matrix(self):
		""" O(n^2) time complexity to reduce the matrix.
			Uses minimal extra space -- space complexity is O(1) """
		extra_cost = 0
		for r in self.matrix:  # reduce n rows
			line_min = np.inf
			for x in r:  # find min, O(n)
				if x < line_min:
					line_min = x
			if line_min != np.inf and line_min > 0:
				for i in range(len(r)):  # reduce row by min, O(n)
					r[i] -= line_min
				extra_cost += line_min
		for i in range(len(self.matrix)):  # reduce n columns
			line_min = np.inf
			for r in self.matrix:  # find min, O(n)
				if r[i] < line_min:
					line_min = r[i]
			if line_min != np.inf and line_min > 0:
				for r in self.matrix:  # reduce column by min, O(n)
					r[i] -= line_min
				extra_cost += line_min
		self.lb += extra_cost
